...
class ObjectContext {
	...
	public function Execute($uriOrDSQueryContinuation) {
		...
		if (is_string($uriOrDSQueryContinuation)) {
			...
			is_object($uriOrDSQueryContinuation) &&
			is_a($uriOrDSQueryContinuation, 'DataServiceQueryContinuation')
		) {
			$queryComponents = $uriOrDSQueryContinuation->CreateQueryComponents();
		} else {
			$queryOperationResponse = new QueryOperationResponse(
				array(),
		...
		if (
			(strpos($queryComponents->Uri, '$inlinecount') !== FALSE)||
		...
		return $this->ExecuteAndProcessResult($httpRequest, Resource::DataServiceVersion_2);
	}
	...
	public function ExecuteAndProcessResult($httpRequest, $dataServiceVersion) {
		$response = $this->ExecuteAndReturnResponse(
			$httpRequest,
			$dataServiceVersion,
			$isError,
			$innerException
